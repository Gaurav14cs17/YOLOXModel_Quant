...
# -*- coding:utf-8 -*-
"""
YOLOX Quantization Aware Training entry point.

Usage:
    # QAT fine-tuning from a pretrained YOLOX-S checkpoint:
    python tools/train_qat.py \
        -f exps/default/yolox_s_qat.py \
        -c /path/to/yolox_s.pth \
        -d 1 -b 16 --fp16

    # With custom QAT schedule via CLI overrides:
    python tools/train_qat.py \
        -f exps/default/yolox_s_qat.py \
        -c /path/to/yolox_s.pth \
        -d 8 -b 64 --fp16 \
...
def make_parser():
    parser = argparse.ArgumentParser("YOLOX QAT Training")
    ...
    parser.add_argument("-c", "--ckpt", default=None, type=str,
                        help="pretrained float model checkpoint for QAT")
